...
enum slap_ErrorCode slap_Riccati_LTI(int N, const Matrix A, const Matrix B,
  ...
  for (--k; k >= 0; --k) {
    ...
    slap_MatMulAdd(Sx, slap_Transpose(A), p[k], 1,
                   1);  // Sx = q + A' * (P*f + p)

    // Control Gradient: Su = r + B' * (P * f + p)
    slap_MatrixCopy(Su, r);  // Su = r
    slap_MatMulAdd(Su, slap_Transpose(B), p[k], 1,
                   1);  // Su = r + B' * (P*f + p)
...
enum slap_ErrorCode slap_RiccatiForwardPass_LTI(
    int N, const Matrix A, const Matrix B, const Matrix f, const Matrix x0,
