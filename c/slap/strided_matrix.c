...
Matrix slap_CreateSubMatrix(Matrix mat, int top_left_row, int top_left_col,
  ...
  SLAP_ASSERT(new_rows >= 0, SLAP_INVALID_DIMENSION, new_mat,
              "CreateSubMatrix: number of rows must be non-negative, got %d",
              new_rows);
  SLAP_ASSERT(new_cols >= 0, SLAP_INVALID_DIMENSION, new_mat,
              "CreateSubMatrix: number of rows must be non-negative, got %d",
              new_cols);
  SLAP_ASSERT(
      slap_CheckInbounds(mat, top_left_row + new_rows - 1,
                         top_left_col + new_cols - 1),
      SLAP_INDEX_OUT_OF_BOUNDS, new_mat,
      "CreateSubMatrix: sub-matrix out of bounds with top-left corner (%d,%d), "
      "bottom-right corner (%d,%d)",
      top_left_row, top_left_col, top_left_row + new_rows - 1,
      top_left_col + new_cols - 1);
