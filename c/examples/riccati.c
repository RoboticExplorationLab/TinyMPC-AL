...
enum slap_ErrorCode tiny_Riccati_LTVf(
    ...
    Matrix* d, Matrix* P, Matrix* p, const Matrix* x_ref, const Matrix* u_ref,
    Matrix S_temp) {
  // Copy terminal cost-to-go
  int k = N;
  slap_MatrixCopy(P[k], Q);
...
//                                  const Matrix Q, const Matrix R, const Matrix
//                                  q, const Matrix r, Matrix* K, Matrix* d,
...
//     slap_MatrixCopy(temp_mm, R);
//     slap_MatMulAdd(temp_mm, slap_Transpose(B[k]), temp_nm, 1, 1); //R + B' *
//     P * B slap_MatMulAdd(temp_nn, P[k+1], A[k], 1, 0);  //temp_nn = P * A
