...
int main() {
  ...
  for (int i = 0; i < NHORIZON; ++i) {
    if (i < NHORIZON - 1) {
      A[i] = slap_MatrixFromArray(NSTATES, NSTATES, &A_data[i*NSTATES*NINPUTS]);
    ...
    state_duals[i] = slap_MatrixFromArray(2 * NSTATES, 1, &state_dual_data[i*NSTATES]);
  }
  ...
  printf("uhrz and xhrz init\n");
  for (int i=0; i<NHORIZON-1; i++) {
    slap_PrintMatrix(Uhrz[i]);
    printf("\n");
  }
  for (int i=0; i<NHORIZON; i++) {
    slap_PrintMatrix(Xhrz[i]);
    printf("\n");
  }
