#include "mpc_ltv_sqrt_bkwd_pass.h"

enum slap_ErrorCode tiny_ConstrainedSqrtBackwardPassLtv(
  ...
  for (int k = N - 2; k >= 0; --k) {
    ...
    // sqrt(Quu)
    Matrix temp_m = slap_NewMatrix(m,1);
    slap_Cholesky(Quu);
    slap_MakeLowerTri(Quu); // Cholesky factor is stored in lower triangle
    slap_QR(Quu, temp_m, temp_m); // QR decomp on Cholesky factor to get sqrt(Quu)

    // sqrt(Qxx)
    Matrix temp_n = slap_NewMatrix(n,1);
    slap_Cholesky(Qxx);
    slap_MakeLowerTri(Qxx); // Cholesky factor is stored in lower triangle
    slap_QR(Qxx, temp_n, temp_n); // QR decomp on Cholesky factor to get sqrt(Qxx)
    ...
    // Find K:
    slap_CholeskySolve(slap_Transpose(Quu_temp),prob->K[k]); // sqrt(Quu)'\Qux 
    slap_CholeskySolve(Quu_temp,prob->K[k]); // K = sqrt(Quu)\(sqrt(Quu)'\Qux)
    slap_ScaleByConst(prob->K[k], -1.0); // K = -sqrt(Quu)\(sqrt(Quu)'\Qux)
    // Find d:
    slap_CholeskySolve(slap_Transpose(Quu_temp),prob->d[k]); // sqrt(Quu)'\Qux 
    slap_CholeskySolve(Quu_temp,prob->d[k]); // K = sqrt(Quu)\(sqrt(Quu)'\Qux)
    slap_ScaleByConst(prob->d[k], -1.0); // K = -sqrt(Quu)\(sqrt(Quu)'\Qux)
...
enum slap_ErrorCode tiny_Sqrt_MpcLtv(Matrix* X, Matrix* U, tiny_ProblemData* prob,
  ...
  for (int iter = 0; iter < solver->max_primal_iters; ++iter) {
    ...
    tiny_ConstrainedSqrtBackwardPassLtv(prob, *solver, model, X, U, &Q_temp,
                                    &ineq_temp);
