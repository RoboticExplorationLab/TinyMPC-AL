 ...
 *
 * Useful for allocating a matrix from a pre-allocated buffer, especially when
 * multiple arrays are allocated from the same chunk of heap-allocated memory.
 *
 * # Example
 * ```c
 ...
 * @param buf The start of the memory buffer where the data for the array should
 * "taken" from. It is shifted forward by the number of bytes used by the
 * matrix.
