...
int main(void) {
  ...
  model.x45min[0] = -model.x45max[0];
  model.x45min[1] = -model.x45max[1];
  model.umin[0] = -model.umax[0];
  model.umin[1] = -model.umax[1];
  ...
  tiny_ReadData_ExtendGoal(file_xref, xref_data, xf_data, NSTATES,
                           NSTATES * NSIM, false);
  tiny_ReadData_ExtendGoal(file_uref, uref_data, uf_data, NINPUTS,
  ...
  for (int k = 0; k < NSIM - NHORIZON; ++k) {
    tiny_LQR_LTVf(NHORIZON - 1, A, B, tiny_GetJacobians, Q, R, q, r, Khist,
                  dhist, Phist, phist, &xref[k], &uref[k], S);

    // Control input: u = uf - d - K*(x - xf)
    slap_MatrixAddition(uhist[k], uref[k], dhist[0],
                        -1);  // u[k] = un[k] - d[k]
