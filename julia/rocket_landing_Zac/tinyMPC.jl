...
function term_cost_expansion(p::NamedTuple, x)
    dx = x - p.Xref[p.N] # delta formulation
    return p.Qf, p.Qf * dx
end
...
function backward_pass!(params, X, U, P, p, d, K, reg, μ, μx, ρ, λ, λc)
    ...
    for k = (N-1):(-1):1
        ...
        Sxx, Sx, Suu, Su = stage_cost_expansion(params, X[k], U[k], k)
        if params.ncu > 0
            # control constraints
        ...
        # Regularization
        Sxx_reg = Sxx + A' * (P[k+1] + reg*I) * A
        Suu_reg = Suu + B' * (P[k+1] + reg*I) * B
        Sux_reg = B' * (P[k+1] + reg*I) * A

        F = cholesky(Symmetric(Suu_reg))
        d[k] = F \ gu
        K[k] = F \ Sux_reg
        ...
        P[k] = Sxx + K[k]'*Suu*K[k] + (A-B*K[k])'*P[k+1]*(A-B*K[k])
        p[k] = Sx - K[k]'*Su + K[k]'*Suu*d[k] + (A - B*K[k])'*(p[k+1] - P[k+1]*B*d[k])
...
function trajectory_AL_cost_grad(params, X, U, μ, μx, ρ, λ, λc)
    @show trajectory_AL_cost(params, X, U, μ, μx, ρ, λ, λc)
    return FiniteDiff.gradient(du -> trajectory_AL_cost(params, X, du, μ, μx, ρ, λ, λc), U)
end
function update_reg(reg,α;reg_min=1e-6,reg_max=1e2)
    if α == 0.0
        if reg == reg_max
...
function forward_pass!(params, reg, X, U, K, d, Qu, Quu, ΔJ, Xn, Un, μ, μx, ρ, λ, λc; max_linesearch_iters=20)
    """LQR forward pass
...
function tiny_solve!(params, X, U, P, p, K, d, Xn, Un; atol=1e-3, max_iters=250, max_inner_iters=10, verbose=true, ρ, ρ_max=1e8, ϕ=10)
